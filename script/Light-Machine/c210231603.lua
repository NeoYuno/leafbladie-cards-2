...
s.listed_names={91998119,58859575}
function s.matfilter(c,fc,sumtype,tp)
	return c:IsAttribute(ATTRIBUTE_LIGHT,fc,sumtype,tp) and c:IsRace(RACE_MACHINE,fc,sumtype,tp) and c:IsType(TYPE_FUSION,fc,sumtype,tp)
...
function s.sprfilter(c)
	return c:IsCode(91998119,58859575) and c:IsAbleToRemoveAsCost()
end
...
function s.spfilter(c,e,tp)
	return c:IsFaceup() and c:IsCanBeEffectTarget(e) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
        and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_MACHINE) and c:IsType(TYPE_FUSION) and c:IsAttackBelow(2800)
end
