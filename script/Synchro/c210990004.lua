...
function s.lvcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsCode,25165047),tp,LOCATION_MZONE,0,1,nil)
end
function s.lvop(e,tp,eg,ep,ev,re,r,rp)
	local lv=Duel.AnnounceLevel(tp,1,2)
	local g=Duel.GetMatchingGroup(aux.FilterFaceupFunction(Card.IsType,TYPE_SYNCHRO),tp,LOCATION_MZONE,0,nil)
	g:Remove(aux.FilterFaceupFunction(Card.IsCode,25165047),nil)
	local tc=g:GetFirst()
