...
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
    if Duel.IsPlayerAffectedByEffect(tp,210924011) and e:GetHandler():GetFlagEffect(id)~=0 then
        return Duel.GetTurnPlayer()==1-tp
    elseif e:GetHandler():GetFlagEffect(id)~=0 then
...
function s.spfilter(c,e,tp)
	return c:IsCode(210924015) and c:IsCanBeSpecialSummoned(e,0,tp,true,true)
end
