...
function s.filter(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0x111)
		and Duel.IsExistingMatchingCard(s.desfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,Group.FromCards(c,e:GetHandler()))
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local dc=Duel.SelectMatchingCard(tp,s.desfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,tc):GetFirst()
...
function s.thfilter(c)
	return c:IsSetCard(0x111) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
