--ボンディング－D2O
-- Custom Version for leafbladie, by MasterQuestMaster.
local s,id=GetID()
...
s.listed_names={58071123,85066822,6022371,43017476} -- Oxygeddon, Water Dragon, Cluster, Duoterion
function s.spcheck(sg,tp)
	return aux.ReleaseCheckMMZ(sg,tp) and sg:IsExists(s.chk,1,nil,sg)
end
function s.chk(c,sg)
	return aux.IsCodeOrChemist(c,58071123) and sg:IsExists(aux.IsCodeOrChemist,2,c,43017476)
end
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroupCost(tp,aux.IsCodeOrChemist,3,true,s.spcheck,nil,43017476,58071123) end
	local sg=Duel.SelectReleaseGroupCost(tp,aux.IsCodeOrChemist,3,3,true,s.spcheck,nil,43017476,58071123)
...
function s.thfilter(c)
	return c:IsCode(85066822,6022371) and c:IsPreviousLocation(LOCATION_ONFIELD)
end
