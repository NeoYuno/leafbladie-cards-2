--Weapon of the Underground - Toy Knife
local s,id=GetID()
...
function s.bccon(e,tp,eg,ep,ev,re,r,rp)
	...
	return Duel.GetAttacker()==c and bc
end
function s.bcfilter(c,def)
	return c:IsFaceup() and c:GetAttack()<=def and c:IsAbleToHand()
end
function s.bctg(e,tp,eg,ep,ev,re,r,rp,chk)
    local def=e:GetHandler():GetEquipTarget():GetDefense()
	if chk==0 then return Duel.IsExistingMatchingCard(s.bcfilter,tp,0,LOCATION_MZONE,1,nil,def) end
	local g=Duel.GetMatchingGroup(s.bcfilter,tp,0,LOCATION_MZONE,nil,def)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,g,#g,0,0)
end
function s.bcop(e,tp,eg,ep,ev,re,r,rp)
    local def=e:GetHandler():GetEquipTarget():GetDefense()
	local g=Duel.GetMatchingGroup(s.bcfilter,tp,0,LOCATION_MZONE,nil,def)
