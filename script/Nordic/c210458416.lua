...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if #fg>0 then loc=loc+LOCATION_DECK end
	local g=Duel.GetMatchingGroup(Card.IsAbleToRemove,tp,loc,0,nil)
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local loc=nil
	if #fg>0 and Duel.SelectYesNo(tp,aux.Stringid(210458410,0)) then
		loc=LOCATION_DECK
	else
	...
	if #rg>0 and Duel.Remove(rg,POS_FACEUP,REASON_EFFECT)~=0 then
        ...
        if og:IsExists(Card.IsPreviousLocation,1,nil,LOCATION_DECK) then fg:GetFirst():RegisterFlagEffect(210458409,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,0) end
		ct=math.min(#og,Duel.GetLocationCount(tp,LOCATION_MZONE))
	...
	e4:SetDescription(aux.Stringid(90207654,2))
